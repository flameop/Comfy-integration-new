...
Version: 2.8.7 - Hid raw text editor for CO_info_links, keeping only hyperlinks
"""
    ...
    def refresh_ui(self):
        ...
        for key, value in self.data.items():
            ...
            if key == "CO_info_links":
                # Create a layout block exclusively containing clean external hyperlinks
                ...
                for segment in url_list_str:
                    segment = segment.strip()
                    if segment:
                        if '|' in segment:
                            link_name, explicit_url = segment.split('|', 1)
                            link_name = link_name.strip()
                            explicit_url = explicit_url.strip()
                        else:
                            link_name = segment
                            explicit_url = segment

                        lbl = QtWidgets.QLabel()
                        lbl.setText(f'  • <a href="{explicit_url}">{link_name}</a>')
                        lbl.setOpenExternalLinks(True)
                        lbl.setStyleSheet("color: #74158ff; font-style: italic;") 
                        url_layout.addWidget(lbl)
                        
                self.form_layout.addRow("Info Links:", url_container)
                # Skip appending to self.entries map so that save_file doesn't search for a non-existent widget
                continue
